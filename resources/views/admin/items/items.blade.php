  ...
  <div class="table-responsive">
    <table class="table table-bordered text-center main-table">
      ...
      <tr>
        ...
        <td>
          <a href="{{ route('edititems', ['id'=>$item->id]) }}" class="btn btn-success"><i class="fa fa-edit"></i> Edit
          </a>
          <a href="{{ route('deleteItem', ['id'=>$item->id]) }}" class="btn btn-danger confirm"><i
              class="fa fa-close"></i> Delete </a>

          @if ($item->regStatus == 0)
          <a href="{{ route('approved', ['id'=>$item->id]) }}" class="btn btn-info activate"><i class="fa fa-check"></i>
            activate </a>
          @endif
